...
| **Staff (ex Camerieri CRM)** | **Una tabella** `staff` con `city_id` FK e `source_candidate_id` nullable per idempotenza promozione da candidato. Naming DB scelto: `staff`; la UI può continuare a parlare di “Camerieri”. **Stato admin (2026-05-02):** CRM camerieri su Supabase (`staff-repository`), sidebar per tutte le sedi attive; promozione da board scrive `staff` e archivia `candidates` (`archivio`). Evoluzione possibile: nota “Promozione in data …” vs `DELETE` candidato — [`PROMPT_CHAT_E4_STAFF_CAMERIERI_SUPABASE.md`](PROMPT_CHAT_E4_STAFF_CAMERIERI_SUPABASE.md). |
