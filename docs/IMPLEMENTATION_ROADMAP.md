...
- [x] **C4 — Invio eventi**  
  ...
  2026-05-09: swap completato verso Supabase Edge Function reale (`supabase/functions/analytics-ingest`); il client invia gli header gateway (`Authorization` + `apikey`) quando l'URL e' `*.functions.supabase.co`. Vedi **L6** sotto.
...
- [x] **L2 — Form contatti (`VITE_CONTACT_ENDPOINT`) + inbox admin** (2026-05-09)  
  Receiver Edge `contact-submissions`: `POST` JSON da `web/components/contact-form.tsx`, validazione server-side, `source` / `status` forzati (`web_contact_form`, `nuovo`), insert `public.contact_messages` (completato). Inbox admin `contact-messages/*` ora su repository Supabase autenticato (`listContactMessages`, `updateContactMessageStatus`, badge count nuovi da DB), con stati robusti `loading/empty/error` e retry, senza mock/localStorage nel percorso runtime. Deploy/env receiver: **`CONTACT_ALLOWED_ORIGINS`**, **`SUPABASE_SERVICE_ROLE_KEY`**, `VITE_CONTACT_ENDPOINT` → URL function (`supabase/README.md`). Prompt operativo inbox: [`PROMPT_CHAT_L2_CONTACT_INBOX_SUPABASE.md`](PROMPT_CHAT_L2_CONTACT_INBOX_SUPABASE.md). Prompt storico receiver: [`PROMPT_CHAT_L2_CONTACT_RECEIVER.md`](PROMPT_CHAT_L2_CONTACT_RECEIVER.md).
...
- [x] **L6 — Analytics ingest reale + timeline campagne + KPI base** (2026-05-09)  
  Pipeline analytics reale end-to-end: Edge Function **`supabase/functions/analytics-ingest`** (POST batch JSON, validazione tollerante per evento, dedup `client_event_id`, ack `accepted_event_ids` + `accepted_count`, mapping `cid -> campaign_id` via RPC `resolve_campaign_id_from_cid`). Timeline campagne (**`first_data_at` / `last_data_at`**) aggiornata server-side dopo ogni evento attribuito tramite RPC **`apply_campaign_event_timeline`** (migrazione **`20260509000010`**, `LEAST/GREATEST` safe sul check `campaigns_data_window_check`). KPI minime per card admin via RPC **`campaign_kpi_aggregates_v1`** (migrazione **`20260509000020`**, conteggi `page_view`, `cta_click`, `careers_form_open`, `careers_submit`, `careers_abandon`, EXECUTE solo `authenticated`). Card mostrano fallback "In attesa dati" quando la campagna non ha eventi. Fix funnel: **`careers_form_open`** emesso solo al primo segnale reale di intent (focus/click sul form, no piu mount). Wiring web in `web/lib/analytics-ingest.ts` (header gateway Supabase quando URL `*.functions.supabase.co`); env esempio aggiornata. Contratti aggiornati: [`ANALYTICS_INGEST_CONTRACT.md`](ANALYTICS_INGEST_CONTRACT.md), [`CAMPAIGNS_CONTRACT.md`](CAMPAIGNS_CONTRACT.md) §5.1. Prompt guida: [`PROMPT_CHAT_ANALYTICS_INGEST_SUPABASE.md`](PROMPT_CHAT_ANALYTICS_INGEST_SUPABASE.md). Smoke E2E: HTTP `2xx` + ack, righe in `public.analytics_events`, `last_data_at` aggiornato.
...
Ultimo aggiornamento checklist (2026-05-09): chiusi **L6 — Analytics ingest reale + timeline campagne + KPI base** e **L2 — Inbox contatti admin su Supabase** (vedi voci dedicate). Pipeline analytics ora end-to-end: web → `analytics-ingest` Edge Function → `public.analytics_events` (append-only, dedup `client_event_id`); risoluzione `cid -> campaign_id` server-side; timeline `campaigns.first_data_at` / `last_data_at` aggiornata via RPC `apply_campaign_event_timeline`; card admin con KPI base via RPC `campaign_kpi_aggregates_v1` + fallback "In attesa dati"; fix funnel `careers_form_open` su intent reale. Inbox contatti admin ora legge/scrive da `public.contact_messages` (nessun mock/localStorage runtime), con `loading/error/retry` e badge sidebar allineato a conteggio DB. Build web/admin verdi, `deno check` ok, smoke HTTP+browser+DB completati. **Restano fuori scope L6 (rimandati):** `registration_duration_seconds`, `candidates_created` per campagna, `conversioni per citta` (KPI avanzate post-MVP). Refactor dashboard admin chiuso (rimozione CMS, candidature/mese, pipeline citta dinamica, fonte traffico con fallback). Aggiornamento precedente (2026-05-07): completato **B3 Site Mode Switch** (`site_mode`, admin Config, gate runtime web; migrazione applicata al DB remoto, build web/admin verdi; resta smoke manuale dei 3 stati). Verificati **`supabase db push`** remoto allineato prima di B3 (include storage **`00170`**) e **smoke Camerieri end-to-end** superato (vedi [`DEVELOPMENT_NOTES.md`](DEVELOPMENT_NOTES.md) § Camerieri). **Stato finale L3:** chiuso come **L3 decision + runbook static-first**; micro-attivita residue solo backlog gia dichiarato (**CMS wiring production-safe**, **Web runtime da DB**), fuori scope primo go-live. Aggiornamento precedente (2026-05-03): **E4 · campagne** — persistenza liste + builder + storage preview; ingest **`first_data_at` / `last_data_at`** documentato come backlog (non admin) → ora chiuso da **L6**. Precedenza (2026-05-02): **Camerieri / `staff`**. Prima (2026-05-01): **`cities`** + board **`candidates`** (**`L5`**), messaggi, **L1**, **E2**, **E5**, **L4**. **Focus successivi suggeriti (post L6):** KPI campagne avanzate (post-MVP, bozza SQL pronta in `supabase/sql/campaign_kpi_advanced_v1_draft.sql`); bucket media CMS (**E3**); **A5**; CRM staff come da [`PROMPT_CHAT_E4_STAFF_CAMERIERI_SUPABASE.md`](PROMPT_CHAT_E4_STAFF_CAMERIERI_SUPABASE.md).
