...
- [x] **L6 Analytics ingest reale + timeline campagne + KPI base** (2026-05-09): la pipeline analytics e' end-to-end. Edge Function **`supabase/functions/analytics-ingest/index.ts`** riceve batch JSON (`events[]`), valida tollerante per evento, risolve `cid -> campaign_id` via RPC esistente `resolve_campaign_id_from_cid`, fa **insert append-only** su `public.analytics_events` con dedup `client_event_id` (unique partial index, gli eventi duplicati vengono comunque ack-ati). Per ogni evento attribuito (non duplicato) aggiorna **`campaigns.first_data_at` / `last_data_at`** tramite RPC **`apply_campaign_event_timeline`** (migrazione `20260509000010`, `LEAST/GREATEST`, preserva `campaigns_data_window_check`). Il client `web/lib/analytics-ingest.ts` invia gli header gateway Supabase (`Authorization: Bearer <anon>` + `apikey`) quando l'URL e' `*.functions.supabase.co`; mock locale resta valido per dev. Ack: `accepted_event_ids` + `accepted_count` (+ `rejected_count` informativo). KPI minime per card admin via RPC **`campaign_kpi_aggregates_v1`** (migrazione `20260509000020`, conteggi base; EXECUTE solo `authenticated`); merge in `campaigns-repository.loadCampaignsForAdmin`; card mostrano fallback **"In attesa dati"** quando la campagna non ha eventi. **Fix funnel:** in `web/components/careers-form.tsx`, `careers_form_open` non parte piu al mount ma su primo segnale reale di intent (`onFocusCapture` + `onPointerDownCapture` sul `<form>`, plus chiamata esplicita nell'onClick del radio sede). Idempotente per attempt via `formOpenTrackedRef`. Contratti aggiornati: [`ANALYTICS_INGEST_CONTRACT.md`](ANALYTICS_INGEST_CONTRACT.md), [`CAMPAIGNS_CONTRACT.md`](CAMPAIGNS_CONTRACT.md) §5.1. Prompt guida: [`PROMPT_CHAT_ANALYTICS_INGEST_SUPABASE.md`](PROMPT_CHAT_ANALYTICS_INGEST_SUPABASE.md). Smoke E2E: HTTP `200` + ack, `analytics_events` popolata con `cid`/UTM, `last_data_at` aggiornato.
...
### Dashboard / Shell
- [x] Dashboard reale: stats (`Messaggi nuovi` da `contact_messages`, `Candidature totali` da `candidates`, `Eventi ingest 30gg` da `analytics_events`); rimossa card "Contenuti CMS".
- [x] Card recruiting: "Candidature per mese" (ultimi 6 mesi su `candidates.created_at`), "Pipeline candidature per città" stacked dinamica (join `cities` + `candidates`, stage `nuovo/colloquio/formazione` + bucket `altro`; mostra solo città con ≥1 candidato).
- [x] Card analytics: "Fonte traffico (candidature)" su `candidates.utm_source` con fallback `Diretto/Sconosciuto`; "Traffico ultimi 3 mesi" reale su `analytics_events` (page_view/cta_click/careers_submit) per mese.
- [x] Empty state condiviso (ChartEmpty), loading via Skeleton, errori non bloccanti via Alert; tutto in `admin/src/components/dashboard/dashboard-repository.ts`.
...
- [x] Navigazione admin persistita in-sessione: al reload viene ripristinata l'ultima view sidebar valida (`static` / `candidates` / `waiters`) tramite `sessionStorage` (`admin:last-page:v1`); in assenza stato valido fallback su `Dashboard › Overview`. La guard su città non più attive resta invariata.
- [x] Pagina **Marketing › Campagne** (`CampagnePage`): lista da **`public.campaigns`**, storage **`campaign-previews`**, salvataggio builder con validazione contratto. **2026-05-09 (L6):** KPI card ora popolate via RPC **`campaign_kpi_aggregates_v1`** (5 conteggi base: `page_view`, `cta_click`, `careers_form_open`, `careers_submit`, `careers_abandon`); merge in `loadCampaignsForAdmin`; banner **"In attesa dati"** quando la campagna non ha ancora eventi. **`first_data_at` / `last_data_at`** aggiornati server-side da `analytics-ingest` via RPC `apply_campaign_event_timeline` (vedi sezione **L6** sopra). **Restano fuori scope L6 (tech debt UX):** `candidates_created` per campagna, `avg_registration_seconds`, `conversioni per citta`, refactor dashboard admin (rimozione voce CMS, "Candidature per mese", pipeline citta dinamica, gestione "Fonte traffico") — vedi `PROMPT_CHAT_ANALYTICS_INGEST_SUPABASE.md` §"Allineamento dashboard admin". Contratto [`CAMPAIGNS_CONTRACT.md`](CAMPAIGNS_CONTRACT.md) §5.1; orchestrazione storica [`PROMPT_CHAT_E4_CAMPAIGNS_SUPABASE.md`](PROMPT_CHAT_E4_CAMPAIGNS_SUPABASE.md).
...
- **Admin last-page policy** (2026-05-09): persistenza ultima view solo in `sessionStorage` (`admin:last-page:v1`), senza TTL applicativo; stato non valido o pagina non più raggiungibile (es. city disattivata) => fallback `dashboard`.
...
- **Campagne `first_data_at` / `last_data_at`** (chiuso 2026-05-09 con **L6**):
  - In **INSERT** da Marketing › Campagna restano **NULL** — comportamento corretto.
  - **Non** vengono mai scritti dall'admin (no shortcut SQL, no "fake analytics" su `campaigns`).
  - Aggiornati esclusivamente server-side dalla Edge Function **`analytics-ingest`** dopo l'insert di un evento attribuito (campaign_id non null), tramite RPC **`public.apply_campaign_event_timeline(uuid, timestamptz, timestamptz)`** (`SECURITY DEFINER`, EXECUTE solo `service_role`, migrazione `20260509000010`). Implementazione `LEAST/GREATEST` → safe su batch out-of-order e preserva `campaigns_data_window_check`.
  - Eventi duplicati per `client_event_id` non aggiornano la timeline (no spurious bumps di `last_data_at`).
...
- **Perimetro residuo:** E3 (Storage **CMS**/media senza baseline); **E4** lato campagne solo su evolutivi post-lancio (KPI avanzate: `candidates_created`, `avg_registration_seconds`, conversioni per citta); L3 come da roadmap.
