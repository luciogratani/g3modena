...
> **Archivio:** non descrive lo stato attuale del repo. **L1** e **E4 sedi** (`public.cities`) sono chiusi; usare il blocco solo come contesto storico o revisione del receiver.

Sei nel monorepo **g3modena** (`admin/`, `web/`, `supabase/`). *(Storico, pre-chiusura L1:)* Roadmap in `docs/IMPLEMENTATION_ROADMAP.md`: **E2 (RLS)**, **E5 (Auth admin)** e **L4** chiusi; passo bloccante allora era **L1 — receiver candidature** (`VITE_CAREER_ENDPOINT`).
...
3. Gestisce **`officeCitySlug`** → **`city_id`** (FK obbligatoria): la tabella `candidates` non ha slug diretto; serve lookup su **`public.cities`** (righe **attive**). Il web usa `application-office-cities.ts` (REST anon + fallback statico); il server deve validare sulla stessa sorgente canonica (**`public.cities`**). *(Storico: prima di E4 si citava un possibile drift con `admin:cities:v1`; oggi sedi e L1 sono allineati al DB.)*
...
- `web/components/careers-form.tsx` (submit e payload JSON/multipart)
...
> **Post‑L1 + storage careers (2026‑05‑01):** bucket `careers-photos` / `careers-cv` (`00140`), receiver `career-submissions` e popolamento `cities` coprono le righe **Allegati** e **Seed cities** qui sotto. La tabella resta come **archivio** dell’analisi pre‑implementazione.

| Area | Dettaglio |
|------|-----------|
| **Allegati** | *(Obsoleto per L1)* Colonne `profile_photo_path`, `cv_path` + upload multipart gestiti da Edge con bucket privati careers (vedi `supabase/README.md` § L1). |
...
| **Seed `cities`** | Il receiver risolve solo slug presenti su **`cities`** con **`is_active = true`**. Operativamente: seed/INSERT o **Config › Sedi** (E4); tabella vuota o slug non allineati ⇒ submit fallisce. |
| **Allineamento slug web ↔ DB** | *(Post E4 sedi)* Web e admin leggono la stessa **`public.cities`**; resta da tenere allineati slug tra deploy e contenuti marketing. |
